...
![Executive Workforce Overview](screenshots/Executive%20Workforce%20Overview.png)
...
![Attrition Risk & Prediction](screenshots/Attrition%20Risk%20%26%20Prediction.png)
...
![Performance & Compensation](screenshots/Performance%20%26%20Compensation.png)
...
![Exit Analysis](screenshots/Exit%20Analysis.png)
...
**High-risk headcount and workforce share** (feeding the risk-tier KPI cards):
```dax
High Risk Headcount =
...
**Average risk score with an explicit inactive-relationship activation** (the risk table is linked to the date table on a relationship that's disabled by default, so it's activated per-query):
```dax
Average Risk Score =
CALCULATE(
